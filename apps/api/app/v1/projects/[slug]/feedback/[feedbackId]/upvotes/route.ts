...
export async function GET(
  ...
  const { data: upvoters, error: upvotersError } =
    await getFeedbackUpvotersById(
      context.params.feedbackId,
      context.params.slug
    );
  ...
  if (upvotersError) {
    return NextResponse.json(
      { error: upvotersError.message },
      { status: upvotersError.status }
    );
  }
...
export async function POST(
  ...
  // Get current user
  const { data: isLoggedIn } = await getCurrentUser();
