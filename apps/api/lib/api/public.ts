...
import type { ChangelogWithAuthorProps, FeedbackWithUserProps } from '../types';

type PublicProjectChangelogResponse = Promise<{
...
export const getPublicProjectChangelogs = (slug: string): PublicProjectChangelogResponse =>
  withProjectAuth(async (user: profiles, project: projects) => {
    ...
    const restructuredData: ChangelogWithAuthorProps[] = changelogs.map(
      (changelog) => ({
        ...changelog,
...
export const subscribeToProjectChangelogs = (slug: string, email: string) =>
  withProjectAuth(async (user: profiles, project: projects) => {
    ...
    if (existing) {
      return {
        data: null,
        error: { message: 'Already subscribed', status: 400 },
      };
    }
...
export const unsubscribeFromProjectChangelogs = (slug: string, subId: string) =>
  withProjectAuth(async (user: profiles, project: projects) => {
    ...
    if (!subscriber) {
      return {
        data: null,
        error: { message: 'Subscriber not found', status: 404 },
      };
    }
