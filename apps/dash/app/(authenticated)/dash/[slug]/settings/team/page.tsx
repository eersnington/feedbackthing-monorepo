...
export default async function TeamSettings({
  ...
  const { data: members, error } = await getProjectMembers(
    params.slug,
    'server'
  );
  ...
  const { data: invites, error: invitesError } = await getProjectInvites(
    params.slug,
    'server'
  );
    ...
    <div className="flex h-full w-full flex-col overflow-y-auto">
      <Card className="flex w-full flex-col">
        <CardHeader>
          ...
          <CardDescription>
            Add or remove users that have access to this project.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <TeamTable
