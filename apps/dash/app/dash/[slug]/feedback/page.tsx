...
export default async function Feedback({
  ...
  const { data: tags, error: tagsError } = await getAllFeedbackTags(
    params.slug,
    'server'
  );
  if (tagsError) {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-2">
        ...
        <Card className=" flex w-full flex-col items-center justify-center p-10 sm:p-20">
          <CardHeader className="items-center text-center ">
            ...
            <CardDescription className="font-light">
              Once somone submits feedback, it will show up here. Make sure to
              share it so others can vote on it!
            </CardDescription>
          </CardHeader>
        </Card>
      )}
      ...
      {feedback.length > 0 && (
        <FeedbackTable fetchedFeedback={feedback} tags={tags} />
      )}
    </div>
  );
}
