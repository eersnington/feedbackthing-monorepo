...
import { ChangelogProps, ChangelogWithAuthorProps, ProfileProps, ProjectProps, ChangelogWithProfile } from '@/lib/types';
...
export const getAllProjectChangelogs = withProjectAuth<ChangelogWithAuthorProps[]>(
  async (user, supabase, project, error) => {
    ...
    // Restructure data
    const restructuredData = changelogs.map((changelog: ChangelogWithProfile) => {
