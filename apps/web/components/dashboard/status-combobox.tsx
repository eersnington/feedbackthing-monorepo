...
export function StatusCombobox({
  ...
  const currentItem = statusOptions.find(
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          variant="outline"
          className={cn(
            'group flex h-8 w-fit items-center justify-between gap-2 font-extralight text-foreground/60',
            triggerClassName
          )}
          size="sm"
        >
            ...
            {statusOptions.map((item) => (
              <CommandItem
                ...
                onSelect={(currentValue) => {
                  ...
                  setOpen(false);
                  onSelect?.(
                    currentItem?.label.toLowerCase() === currentValue
                      ? ''
                      : currentValue
                  );
                }}
              ...
              >
                ...
                <Check
                  className={cn(
                    'ml-auto h-4 w-4',
                    status.toLowerCase() === item.label.toLowerCase()
                      ? 'opacity-100'
                      : 'opacity-0'
                  )}
                />
