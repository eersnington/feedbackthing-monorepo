...
export function TagCombobox({
  ...
  const EmptyItem = () => {
    ...
    return (
      <>
        ...
        >
          Create tag:&nbsp;
          <span className="font-extralight text-foreground/60">
            &quot;{search}&quot;
          </span>
        </CommandItem>
      </>
  ...
  const currentTags = projectTags.filter((item) =>
    tags.includes(item.value.toLowerCase())
  );
                ...
                <div
                  ...
                  style={{
                    backgroundColor: projectTags.find(
                      (item) =>
                        item.value.toLowerCase() === tags[0].toLowerCase()
                    )?.color,
                  }}
                />
                {/* Tag name */}
          ...
          <CommandGroup>
            {projectTags.map((item) => (
                ...
                onSelect={(currentValue) => {
                  setTags((prev) => {
                    if (prev.includes(currentValue)) {
                      return prev.filter((tag) => tag !== currentValue);
                    }
                    return [...prev, currentValue];
                  });
                  if (onSelect) {
                    const lowerCaseTags = currentTags?.map((tag) =>
                      tag.value.toLowerCase()
                    );
                    const includesCurrentValue =
                      lowerCaseTags?.includes(currentValue);

                    if (lowerCaseTags && includesCurrentValue) {
                      const filteredTags = lowerCaseTags.filter(
              ...
              >
                {/* Tag color */}
                <div
                  className="mt-[1px] h-2 w-2 rounded-full"
            ...
            <EmptyItem />
            <CommandEmpty className="w-full p-2 text-center font-extralight text-muted-foreground text-xs">
